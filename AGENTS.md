...
## 开发台账：所有 Agent 必须遵循
- 开始个人功能开发、排障或交接前，读取 `docs/DEVELOPMENT_STATUS.md` 当前状态和对应稳定ID；它是整体进度唯一入口，历史快照不得作为当前部署事实。
- 开工关联台账ID、Agent Ops任务、fork Issue和任务分支；遇到新问题先登记。阶段变化、范围调整、测试、review、合并、部署或阻塞时同步条目和变更记录，不能只在聊天汇报。
- 完成条目需写明证据层级、验收范围、reviewer、PR及来源；保留已完成和复发历史。若只改文档，明确运行版本未变，不为文档提交重装App。
- 用户的自动化与Obsidian双向联动需求已记录为planned；按台账逐项实施，禁止把需求记录视为真实任务执行、全库导入或批量修改共享提示词的授权。未来获准迁移仅明确Codex来源且项目可判定的清单，历史执行绑定允许为空。
- 使用本文件 `AGENTS.md` 作为标准Agent入口，不另建重复的 `agent.md`。维护部署文档的实际运行信息与开发台账之间的区分。
