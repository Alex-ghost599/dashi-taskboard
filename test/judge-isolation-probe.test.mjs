...
readline.createInterface({input:process.stdin}).on('line',async line=>{
 ...
 for(let i=0;i<6;i++){
 const r=await fetch(url+'/responses',{method:'POST',body:JSON.stringify({model:'gpt-5.3-codex-spark',reasoning:{effort:'low'},tools,input})});
 ...
 if(item.type==='function_call')input.push({type:'function_call_output',call_id:item.call_id,output:mode==='success'?'ok':'unsupported call: '+item.name});
 }
...
test('probe rejects all five calls, uses isolated spawn cwd/env and removes only its temporary runtime', async t=>{
  ...
  assert.equal(result.syntheticRequests,6);assert.equal(result.authorizesDispatch,false);
...
for(const mode of ['sandbox','tools','success'])test(`probe fails closed for ${mode} mismatch`,async t=>{
