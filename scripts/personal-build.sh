...
npm run tauri -- build --target aarch64-apple-darwin --bundles app --features personal --no-sign --config src-tauri/tauri.personal.conf.json
# Sign a metadata-free copy outside Documents/iCloud, which can reattach FinderInfo.
artifact_dir=$(mktemp -d "$HOME/Library/Caches/dashi-personal-build.XXXXXX")
artifact="$artifact_dir/Dashi Taskboard Personal.app"
ditto --noextattr --norsrc 'src-tauri/target/aarch64-apple-darwin/release/bundle/macos/Dashi Taskboard Personal.app' "$artifact"
xattr -cr "$artifact"
codesign --force --deep --sign - "$artifact"
codesign --verify --deep --strict "$artifact"
mkdir -p .local-deploy
printf '%s\n' "$artifact" > .local-deploy/artifact-path
