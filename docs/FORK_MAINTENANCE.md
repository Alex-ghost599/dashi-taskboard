...
## 自动化候选基础（2026-09-15）
#16 新增确定性候选摘要与独立 Judge 缓存/租约存储；不连接旧 scheduled，不授予执行权限。同步上游时保留语义/授权版本分离、未知结果不自动重放、持久重试上限及跨进程争抢测试。接口和生产接入缺口见 AUTOMATION_CANDIDATES.md；#16 待 #17 联合验收，不因基础模块合入而关闭。
