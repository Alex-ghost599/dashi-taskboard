...
## #25A 旧scheduled精确归属
修复记录ID缺失后回退/新建、同名第一条和不同项目覆写风险；元数据不符或歧义时无写入，暂停意图继续保留并标记未确认。34项协调/暂停回归通过，独立终审Critical0/Important0；并入索引前的本分支全量Node538通过/1skip。未迁移真实scheduled或启用新执行；备份/读回/单协调器及UI仍待，详见LEGACY_SCHEDULE_OWNERSHIP.md，#25保持开放。
