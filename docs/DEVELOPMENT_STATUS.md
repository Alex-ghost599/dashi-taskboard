...
## OBS-01 身份及受管字段契约 v1（#27）
补明来源状态/证据链接与可信执行回执区别，列出源身份、可编辑源内容、派生索引和独立控制数据的管理规则。中央协议及已配置写入者盘点为依据，未知字段保留，未知来源不猜。当前没有写回实现或共享指南修改；联合预览已由 #28 / PR #55 合并，实际同步和指导落地仍由 #29/#30 推进。
...
暂停更新的响应不能证明计划已保存为PAUSED。现有本地injector路径增加同ID状态重读；响应ID不符、读取失败或仍ACTIVE均保留暂停未确认状态，失效请求返回stale。归属检查沿用reconcile；远端独立分支未改。专项回归覆盖ACK与实际状态矛盾、读失败、错误ID及读回期间请求失效。此阶段没有运行真实计划迁移，#25仍开放。
...
## #34 手动会话建卡与执行绑定（候选，未部署）

手动建卡创建进行中卡片、记录可信来源，默认提示复用同会话已有卡；明确选择后才允许新增。此默认策略为当前实现选择，尚未获得用户对多卡偏好的单独确认。同请求重试复用已建卡，不覆盖后续编辑或重复发送创建事件；撤销关联保留来源历史与卡片，删除卡片也保留去重凭据，旧请求不能复建。

执行绑定独立于来源记录。输入精确会话 ID，核验 host/project/cwd/thread 后预览，再次核验并显式确认；解绑单独确认。历史来源不迁移为执行绑定，普通编辑不能改变执行目标。绑定版本与在途尝试使用协调事务；UNKNOWN 占用时拒绝换绑。项目目录被清空后仍可查看与解绑。

只读 CDP 适配器检查 loopback、指定进程所有权、可信顶层页面及读取前后的 frame/loader/owner。仅调用 thread/read（不含 turns）及项目归属读取。未配置或无法核验时拒绝，不根据标题、当前选中项目或客户端 host-runtime 推断身份。此流程不发送任务、调用执行模型或启用自动认领。
