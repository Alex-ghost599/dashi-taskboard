...
候选提供手动会话建卡和显式执行绑定。默认没有桌面 resolver；普通看板可以运行，首次建卡或绑定核验会报告不可用。已成功建卡的幂等重试、来源关系查询与撤销不需要桌面在线。

只读桌面适配仅支持 personal macOS 入口，通过服务端环境显式设置 `CODEX_TASKBOARD_BINDING_CDP_PORT` 与 `CODEX_TASKBOARD_BINDING_CODEX_EXECUTABLE`。操作员须先验证官方可执行文件和既有 loopback CDP 端口；本入口只附加，不启动、重启 Codex，也不安装看板入口；核验时通过 CDP 执行固定只读表达式。不得从客户端输入、进程完整命令行或页面标题自动推断配置。正式 App 尚未配置此功能，Finder 启动时的持久配置交付也尚未验收。

适配器检查精确任务与原生项目归属，歧义、缺失或不一致时拒绝。桌面查询应用层最多 4 个并发等待、5 秒超时；adapter 响应 AbortSignal，CDP 自身 4.5 秒截止。已进入 renderer 的只读请求依靠 3 秒计时清理，关闭连接不代表立即撤销。未验证非合作 adapter 的底层工作量硬限制。

存储变化：

- 主任务库新增 `manual_card_creations`，卡片与去重凭据同事务保存。撤销保留凭据与来源证据；卡片删除后凭据仍在，旧请求报已删除。
- 服务首次访问绑定控制路径时，在主任务库同目录创建或打开 `execution-control.sqlite`，升级至 schema 5，新增专用绑定及修订表。历史来源不迁移为已授权绑定，保留已有 attempt/UNKNOWN 状态。
- 协调连接附加主任务库，在短事务内核对版本、项目、目录；附加连接具有写能力，当前 SQL 只查询主任务库，不能称为数据库强制只读。事务内不等待桌面或模型。`task.version` 不覆盖评论/依赖等完整语义，未来派发仍需完整快照核验。

部署前必须备份主任务库及已有协调库，并验证隔离恢复。当前安装器仅在线备份主任务库，协调库备份须在正式部署前补齐。旧程序不能直接打开 schema 5 协调库；回滚应配对程序和协调库，并保留、对账升级后新增记录，不能覆盖 UNKNOWN 或执行历史。未完成这些条件时不得宣称正式部署完成。

已验证：隔离浏览器使用模拟来源完成建卡、重复提示、撤销刷新持久化、执行绑定确认与解绑；测试服务已退出，正式数据未迁移。Node 602 通过/1 跳过、组件 27/27，typecheck/build:web 通过；独立增量审核无未解决 Critical/Important。真实 CDP getter、正式 App 配置及安装验收仍待。
